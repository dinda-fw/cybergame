      ...
      <div className="content-area flex-column" style={{ padding: '0.5rem', gap: '0.5rem', flexGrow: 1 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '0.5rem' }}>
          <div style={{ background: 'rgba(255, 165, 0, 0.1)', borderLeft: '3px solid #F59E0B', padding: '10px', borderRadius: '4px' }}>
            <p style={{ margin: 0, fontSize: '0.85rem', lineHeight: '1.4' }}><span style={{fontWeight: 'bold', color: '#F59E0B'}}>Studi Kasus:</span> Lab Komputer sekolah sedang diserang oleh berbagai macam virus! Kamu harus bertindak sebagai detektif untuk memeriksa kelima komputer di lab ini satu per satu.</p>
          </div>
          <div style={{ background: 'rgba(0, 240, 255, 0.1)', borderLeft: '3px solid var(--primary-color)', padding: '10px', borderRadius: '4px' }}>
            <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: 'bold', lineHeight: '1.4' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexGrow: 1, height: '100%', overflow: 'hidden' }}>
          ...
          <div style={{ width: '35%', background: 'rgba(11, 16, 30, 0.8)', border: '1px solid var(--border-blue)', borderRadius: '12px', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
            <div style={{ padding: '10px', background: 'rgba(0, 240, 255, 0.1)', borderBottom: '1px solid var(--border-blue)', fontWeight: 'bold', fontSize: '0.85rem' }}>
              DAFTAR KOMPUTER
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                      <span style={{ fontSize: '0.8rem', fontWeight: 'bold', color: isSelected ? 'var(--primary-color)' : 'white' }}>{pc.name.split(' ')[0]}</span>
                      <span style={{ fontSize: '0.65rem', color: isResolved && !processedInfo.isCorrect ? 'var(--danger-red)' : 'var(--text-muted)' }}>{statusText}</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{ width: '65%', background: '#0a0f1a', border: '1px solid var(--border-blue)', borderRadius: '12px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
            <div style={{ padding: '10px', background: 'rgba(0, 240, 255, 0.1)', borderBottom: '1px solid var(--border-blue)', fontWeight: 'bold', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {selectedPC && (
              <div style={{ padding: '15px', display: 'flex', flexDirection: 'column', gap: '15px', flexGrow: 1, overflowY: 'auto' }}>
                ...
                <div style={{ background: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.2)', padding: '10px', borderRadius: '8px', marginTop: 'auto' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '5px', color: 'var(--danger-red)', fontSize: '0.8rem', fontWeight: 'bold' }}>
                    <HardDrive size={14} /> LAPORAN KELUHAN
                  </div>
                  <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-main)', fontStyle: 'italic', lineHeight: '1.4' }}>
                    "{selectedPC.symptom}"
                  </p>
                </div>
              </div>
              ...
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(5px)', borderRadius: '12px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '20px', zIndex: 10 }}>
                {status === 'correct' && (
                  <>
                    ...
                    <p style={{ fontSize: '0.85rem', textAlign: 'center', margin: '10px 0' }}>{selectedPC?.explanation || 'Tindakan yang tepat! Sistem berhasil diamankan.'}</p>
                ...
                {status === 'success' && (
                  <>
                    ...
                    <p style={{ fontSize: '0.85rem', textAlign: 'center', margin: '10px 0' }}>Hore! Kamu berhasil memeriksa seluruh komputer di lab.</p>
                ...
                {status === 'fail' && (
                  <>
                    ...
                    <p style={{ fontSize: '0.85rem', textAlign: 'center', margin: '10px 0' }}>Tindakanmu kurang tepat untuk komputer ini.</p>
                    <p style={{ fontSize: '0.75rem', textAlign: 'center', color: 'var(--text-muted)' }}>{selectedPC?.explanation}</p>
                ...
                {status === 'timeUp' && (
                  <>
                    ...
                    <p style={{ fontSize: '0.85rem', textAlign: 'center', margin: '10px 0' }}>Waktu habis! Virus telah merusak semua komputer.</p>
        ...
        <div style={{ display: 'flex', gap: '10px', marginTop: 'auto' }}>
          <button 
            ...
            style={{ flex: 1, background: 'rgba(239, 68, 68, 0.1)', border: '1px solid var(--danger-red)', color: 'white', padding: '12px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', fontSize: '0.8rem', fontWeight: 'bold', opacity: status !== 'playing' ? 0.5 : 1 }}
          ...
          <button 
            disabled={status !== 'playing'}
            className="btn hover-glow" 
            style={{ flex: 1, background: 'rgba(245, 158, 11, 0.1)', border: '1px solid var(--warning-yellow)', color: 'white', padding: '12px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', fontSize: '0.8rem', fontWeight: 'bold', opacity: status !== 'playing' ? 0.5 : 1 }}
          ...
          <button 
            disabled={status !== 'playing'}
            className="btn hover-glow" 
            style={{ flex: 1, background: 'rgba(16, 185, 129, 0.1)', border: '1px solid var(--success-green)', color: 'white', padding: '12px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', fontSize: '0.8rem', fontWeight: 'bold', opacity: status !== 'playing' ? 0.5 : 1 }}
